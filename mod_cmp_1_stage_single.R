...
create_sample <- function(prm){
  ...
  df <- df %>% add_column(t0  = runif(n, min=t0_min, max=t0_max )) %>%
    ...
    mutate(ti = exp((psi_star_vec[[1]])*a_0)*t0)
...
calculate_tau_k <- function(df, prm, psi_hat_vec,  a_k=0, ...){
  ...
  df <- df %>% mutate(t0_hat_k = exp(-(psi_hat_0)*a_0)*ti)
...
calculate_variance <- function(df, psi_hat_vec, prm){
  ...
  Jtt <- sum(D_psi_vec*D_psi_vec*fit*(1-fit))
  ...
  Jtb <- t(sapply(1:dim(D)[[2]], function(j){
            sum(D_psi_vec*D[,j]*fit*(1-fit))
          }))
