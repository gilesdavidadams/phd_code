  ...
  with(prm, {
    ...
    df <- tibble(a_0 = c(rep(0, a0_size), rep(1, n - a0_size))) %>%
      ...
      mutate(ti = 0, t0_rsd = t0,
             ...
             g_psi = psi_star_vec[[2]] + x*psi_star_vec[[3]],
             temp = t0_rsd,
...
calculate_tau_k <- function(df, prm, psi_hat_vec,  a_k=0, ...){
  with(prm, {
    df <- df %>% mutate(ti_temp = pmax(ti - t_a, 0),
                        g_psi = psi_hat_vec[[2]] + psi_hat_vec[[3]]*x, 
...
calculate_tau_rsd_m <- function(df, prm, 
  ...
  with(prm, {
    if (m == 0) {
      ...
      df <- df %>% mutate(g_psi = psi_hat_vec[[2]] + psi_hat_vec[[3]]*x,
...
calculate_score <- function(df, prm, psi_hat_vec){
  ...
  S_2 <- with(df_temp, sum((a_1 - fit_a1)*tau_k*x))
...
calculate_jacobian <- function(df, prm, psi_hat_vec){
  ...
  jacobi_vec <- with(df_0, c(
    sum((a_0 - fit_a0)*tau_rsd*a_0), 0, 0,
    0,0,0,
    0,0,0
  ))
  ...
  jacobi_vec <- jacobi_vec + with(df_1, c(
    0,0,0,
    0, sum((a_1 - fit_a1)*tau_rsd*a_1), sum((a_1 - fit_a1)*tau_rsd*a_1*x),
    0, sum((a_1 - fit_a1)*tau_rsd*a_1*x), sum((a_1 - fit_a1)*tau_rsd*a_1*x*x)
    ))
  
  return(matrix(jacobi_vec, byrow=T, nrow=3))
...
calculate_variance <- function(df, prm, psi_hat_vec){
  ...
  D_theta_vec <- c(rep(1, n_0), rep(0,n_1),
                   rep(0, n_0), rep(1, n_1),
                   rep(0, n_0), df_1$x)
  D_theta <- matrix(D_theta_vec, byrow=F, ncol=3)
...
nr_run <- function(psi_star_0=log(2), 
  ...
  prm <- list(psi_star_0=psi_star_0,
              ...
              psi_star_2=psi_star_2,
              psi_star_vec=c(psi_star_0, psi_star_1, psi_star_2),
              t_a=t_a,
              t0_min=t0_min,
              t0_max=t0_max,
              n=n,
