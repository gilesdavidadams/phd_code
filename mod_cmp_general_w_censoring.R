...
create_sample <- function(prm){
  ...
  df <- tibble(x = c(rep(0, n_s*(2^a_vars)), rep(1, n_s*(2^a_vars)))) %>%
          add_column(C_i = prm$censor_date)
...
calculate_tau_k <- function(df, psi_hat_vec, prm,  a_k=0, ...){
  ...
  with(prm, {
    ...
    for (i in (a_k+1):length(t_a_vec)){
      ...
      if (prm$censor) {
        t_next <- ifelse(i < length(t_a_vec), t_a_vec[[i+1]] , prm$censor_date)
        df <- df %>% mutate(
          C_psi = ifelse(g_psi < 0, g_psi, exp(-g_psi)),
      ...
      for(col_counter in 1:jacobi_dim) {
        ...
        for (a_curr in (0:(length(prm$t_a_vec)-1))){
          if (beta_track_row[[a_curr+1]] == i) {
            ...
            if(prm$censor) { 
              df_k <- df_k %>% mutate(Si_censor = delta_k*dC_dpsi + 
                                        (1-delta_k)*Si_temp)
...
nr_run <- function(prm){
  ...
  print_results(nr_out=nr_out, prm=prm)
  
  return(nr_out)
}

print_results <- function(nr_out, prm){
  cat("---------------------------", "\n", prm$sim_label, "\n",
