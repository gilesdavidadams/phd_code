...
create_sample <- function(prm){
  ...
  df <- df %>% add_column(t0  = runif(n, min=t0_min, max=t0_max )) %>%
    ...
    mutate(ti = exp((psi_star_vec[[1]]*(1-x) + psi_star_vec[[2]]*x)*a_0)*t0)
...
calculate_tau_k <- function(df, prm, psi_hat_vec,  a_k=0, ...){
  ...
  psi_hat_0 <- psi_hat_vec[[1]]
  psi_hat_1 <- psi_hat_vec[[2]]
  
  df <- df %>% mutate(t0_hat_k = exp(-(psi_hat_0*(1-x) + psi_hat_1*x)*a_0)*ti)
...
calculate_score <- function(df, prm, psi_hat_vec){
  ...
  S_1mx <- with(df_temp, sum((a_0 - fit_a0)*t0_hat_k*(1-x)))
...
calculate_jacobian <- function(df, prm, psi_hat_vec){
  ...
  jacobi_vec <- c(
    with(df_temp, sum((a_0 - fit_a0)*t0_hat_k*a_0*(1-x)*(1-x))),
    with(df_temp, sum((a_0 - fit_a0)*t0_hat_k*a_0*x*(1-x))),
    with(df_temp, sum((a_0 - fit_a0)*t0_hat_k*a_0*x*(1-x))),
    with(df_temp, sum((a_0 - fit_a0)*t0_hat_k*a_0*x*x))
  )
...
calculate_variance <- function(df, psi_hat_vec, prm){
  ...
  D_psi_vec <- with(df_temp, c(t0_hat_k*(1-x), t0_hat_k*x))
...
nr_run <- function(psi_star_0, 
  ...
  prm <- list(psi_star_0=psi_star_0,
              ...
              psi_star_vec=c(psi_star_0, 
                             psi_star_0 + psi_star_1),
              prb_x = prb_x,
              prb_a_x0 = prb_a_x0,
              prb_a_x1 = prb_a_x1,
