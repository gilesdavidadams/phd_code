...
fit_treatment_models <- function(df, prm){
  ...
  df <- df %>% mutate(fit_0 = glm(a_0 ~ 0 + omx + x, family=binomial)$fitted.values)
  
  if(length(prm$t_a_vec) > 1){
    for(i in 1:(length(prm$t_a_vec)-1)){
      mdl_formula <- paste0("a_", i, "~ 0 + omx + x +  ",
...
nr_run <- function(prm){
  ...
  #psi_star_vec <- c()
  #for (i in 1:length(prm$psi_x_star)){
  #  psi_star_vec <- c(psi_star_vec, prm$psi_x_star[[i]], prm$psi_1mx_star[[i]])
  #}
  #prm$psi_star_vec <- psi_star_vec
  
  #prm$psi_star_vec <- c(prm$psi_1mx_star, prm$psi_x_star)
...
prm$psi_star_vec <- c(log(1.5), log(2))
